...
| 노드 A flannel-wg 주소 | `10.42.0.0` (= 노드 A `.spec.podCIDR` `10.42.0.0/24`의 **네트워크 주소**) | `allow-apiserver-webhook`(`cert-manager` 10250 · `external-secrets` 10250 · `cnpg-system` 9443) — apiserver가 **다른 노드의** 파드 IP로 직접 dial할 때의 출발 IP(같은 노드 파드로 가는 경로는 `cni0`를 타고 `src`가 `10.42.0.1`(브리지 주소)이라 flannel 주소가 아니다 — 2026-09-18 실측. cert-manager·cnpg-system = VD-W 실측 2026-09-09 · external-secrets = T045 G1p D6 = A 2026-09-17, 값의 검증 수단은 머지 전 노드 A 실측(`flannel-wg` 장치 주소 · **다른 노드(B)의 파드 IP**로의 `ip route get` `src`)과 하네스 `np-set-5`이며, **2026-09-18 실측으로 확정됐다 — 세 값 일치 = `10.42.0.0`**. 기록은 모노레포 런북 `docs/runbooks/bootstrap.md` §3 T045 절). 노드 재조인·재이미지 시 `.spec.podCIDR`과 **재대조**할 것 — 리스가 바뀌면 이 규칙은 조용히 무력해진다 |
