...
컴포넌트마다 `kustomization.yaml`(helmCharts values 인라인 또는 순수 매니페스트). 계약 §sync-wave 단일 표에 없는 디렉터리를 만들면 validate가 실패한다. 디렉터리 20개 중 19개는 T003이 뼈대로 만들었고(`secret-stores/`만 T045 G2에서 신설), 그 뒤 컴포넌트 태스크가 하나씩 채우고 있다.

**채워진 컴포넌트**(2026-09-18 기준 10개) — 각 디렉터리의 `README.md`가 운영자 절차 정본이다.
